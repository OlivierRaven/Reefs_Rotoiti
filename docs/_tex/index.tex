...
no statistical room for a BACI signal (Figure~\ref{fig-power-plots}).
For kōura CPUE, simulation-based power analysis showed that detecting
the observed BACI effect size (β = 0.31 log-units) at 80 \% power would
...
pairs (Figure~\ref{fig-cpue-power-plots}). Post-construction power
...
ineffectiveness of the stone piles. Habitat establishment for freshwater
crayfish is a slow process: Johnsen \& Taugbøl
(\citeproc{ref-Johnsen2008}{2008}) reported that crayfish abundance at a
stone site addition site took 13 years to reach levels comparable to a
control site, and freshwater invertebrate colonisation studies generally
show treatment effects emerging over two to three years
(\citeproc{ref-BRON}{\textbf{BRON?}}). Following reconstruction of the
Rotorua lakefront, which included rocky kōura habitat only 17 kōura were
recorded after 28 months, compared to 57 removed before construction,
with no kōura observed 13 months after completion and slow recolonation
was likely the effect of catfish predation pressure
(\citeproc{ref-Kusabs2023}{Kusabs, 2023}). This suggests 18 months
represents a minimum rather than a sufficient monitoring duration for
detectecting population-level responses to habitat addition.

The null result is further expected from the study's statistical power.
Achieving 80 \% power to detect the observed kōura CPUE effect with five
site pairs would require an effect approximately six times larger than
currently observed, and power remained below 26\% across all simulated
scenarios. Absence of a significant BACI effect therefore reflects the
variance structure and sample size of the design rather than evidience
that stone piles are ineffective. Pre-construction kōura occupancy
ranged from 81-89 \% across all site types leaving little statistical
room to detect a treatment effect through occupancy modeling. High
occupancy indicates kōura were already accessing the littoral zone
before deployment likely to use as foraging habitat as littoral zone is
most productive zone of the lake (\citeproc{ref-Devcich1979}{Devcich,
1979}). Occupancy is therefore insensitive to the marginal improvement
stone piles add to the littoral zone, abundance and biomass are
better-suited response variables for this intervention. For kōura CPUE
the simulation-based power analysis showed that for a power of 80 \% 167
pairs are needed which is not duable with the current monitoring design
but realisticly the stone piles deployed during this study only covered
in total \textasciitilde60 m2 which is compared to the 60 km length of
the shore line hardly noticeable therefore we expect before we can see a
lake wide effect deplometn of 167 stone pile sites does not sound
strange.

The first post-deployment monitoring round (Autumn 20205) showed
eleveated kōura CPUE across all site types, which may have obscured any
early construction effect in the BACI design. This cross-site spike is
consistent with seasonal movement patterns in kōura, and because it
occurred equally at stone pile and control sites it cannot be attributed
to the structures. Wide confidence intervals throughout reflect genuine
biological variability emerged from the small number of modified sites
(\emph{n} = 5).
...
Closer observations of the the algea growing on the stone piles revield
a varaiaty of cyanobacteria, green algea, and varies species like
tardigrade, snails, and crustaceans living on the stones. As the stones
were sourced from a query freshly broken rock provides clean surface
where algea will settle and overtime sucssesion of grazers and other
species will devalop who could be a food source for koura again.
...
During night spotlighting on stone pile sites 3 and 5 on 11/08/2026 one
big kōura was spotted in the stones at site 3 and three small kōura were
seen in and around the stones at site 5. Proving that kōura were still
active in and around the stone piles just in lower numbers during
winter.
...
or cleaning of the crevices from sediments could potentially restore
...
Despite non-significant BACI results, after-deployment contrasts showed
a consistently positive direction for stone pile relative to control
sites in koura abundance, and stone pile sites were indistinguishable
from reference sites in
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Stone_pile_devalopment.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{outputs/fig-power-plots.png}}

}

\caption{\label{fig-power-plots}Simulation-based occupancy BACI power
analysis. Left: power heatmap across site pairs and monitoring rounds.
Right: power curves by number of after-period rounds. Dashed line marks
80\% power threshold.}
...
\pandocbounded{\includegraphics[keepaspectratio]{outputs/fig-cpue-power-plots.png}}

}

\caption{\label{fig-cpue-power-plots}Simulation-based kōura CPUE BACI
power analysis (M2aak). Left: power heatmap across site pairs and
monitoring rounds. Right: power curves by number of after-period rounds.
Dashed line marks 80\% power threshold.}
