...
run_contrasts <- function(fit, model_data, model_label, include_season = TRUE) {
  ...
  site_colours <- c(Control = "#E69F00", Stone_pile = "#D55E00",
                    Reference1 = "#0072B2", Reference2 = "#56B4E9")
  site_shapes  <- c(Control = 24, Stone_pile = 21,
...
results_warm$cell_summary |>
  ...
  knitr::kable(
    align   = c("l", "l", "l", "c"),
    caption = paste0(
      "Posterior kōura occupancy probability (\\u03c8) from the warm-season ",
      "occupancy model (Summer and Autumn rounds only). Values are population-level ",
      "estimates averaged over random site effects, with 95\\% credible intervals. ",
      "Autumn has no Before period as baseline monitoring was conducted in summer only."
    )
  )
```
...
ggsave(file.path("outputs", "fig-occ-power-heatmap.png"),
...
ggsave(file.path("outputs", "fig-occ-power-curves.png"),
       p_lines, width = 8, height = 5, dpi = 300)
```

```{r}
#| label: fig-power-plots
#| fig-cap: "Occupancy BACI power analysis: power heatmap (left) and power curves by monitoring rounds (right). Dashed line marks 80% power threshold."
#| fig-width: 12
#| fig-height: 5
p_heat + p_lines
```
...
ggsave(file.path("outputs", "fig-cpue-power-heatmap.png"),
...
ggsave(file.path("outputs", "fig-cpue-power-curves.png"),
       p_lines_cpue, width = 8, height = 5, dpi = 300)
```

```{r}
#| label: fig-cpue-power-plots
#| fig-cap: "Kōura CPUE BACI power analysis (M2aak): power heatmap (left) and power curves by monitoring rounds (right). Dashed line marks 80% power threshold."
#| fig-width: 12
#| fig-height: 5
p_heat_cpue + p_lines_cpue
```
...
#| fig-cap: "Projected kōura CPUE BACI effect size trajectory under continued monitoring. The green line shows the linear projection from the current BACI (0.311 log-units) growing at the M2bak divergence rate (+0.243 log-units per quarterly round). The dashed horizontal line shows the 80% power detection threshold for 5 site pairs (1.80 log-units, 6.0×). The dotted vertical line marks where the projection crosses the threshold (~6 additional quarterly rounds, ~1.5 years). The shaded ribbon shows propagated uncertainty. Note: the projection assumes linear growth; saturation of stone pile carrying capacity is expected to slow the trajectory."
