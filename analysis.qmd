...
# Single lookup function — partial term matching so you don't need full term names
...
# Filter to study period — start from July 2024 to include full Before period
...
#| fig-width: 12
#| fig-height: 8
#| fig-cap: "Lake Rotoiti dissolved oxygen (mg/L, top) and water temperature (°C, bottom) from the continuous buoy record across the study period. Vertical dashed lines mark each monitoring visit; the solid orange line marks stone pile completion (14 February 2025)."

stone_pile_date <- as.Date("2025-02-14")

# One median date per monitoring round across all sites
monitoring_lines <- visit_dates |>
  dplyr::group_by(monitoring) |>
  dplyr::summarise(date = median(visit_date), .groups = "drop")

# Daily mean per 1-m depth bin
...
# X limits: up to 2 weeks either side of the monitoring window,
# clamped to the actual buoy record so no empty space appears
x_lims <- c(
...
p_temp <- ggplot(buoy_daily, aes(x = Date, y = Depth_bin, fill = Temp_mean)) +
  ...
  labs(title = "Lake Rotoiti water temperature (°C)", x = NULL, y = "Depth (m)") +
  theme_bw() +
...
p_do <- ggplot(buoy_daily, aes(x = Date, y = Depth_bin, fill = DO_mean)) +
  ...
  labs(title = "Lake Rotoiti dissolved oxygen (mg/L)", x = NULL, y = "Depth (m)") +
  theme_bw() +
  theme(plot.title = element_text(size = 11))
...
#| fig-height: 3
#| dpi: 300
#| fig-cap: "Mean kōura CPUE per monitoring round (± 95% CI, log scale) by site type. Shading distinguishes the pre-construction period (Before) from post-construction monitoring (After)."
...
fit_rq2b <- function(species, response_type) {
  ...
  form <- paste0("`", response_col, "` ~ site * monitoring_int + (1 | pair_id/site_id)")
...
forest_data <- tidy_all %>%
  ...
  dplyr::mutate(
    ...
    group = dplyr::case_when(
      RQ == "RQ1"  & comparison %in% c("vs_control", "treatment") ~ "Baseline check (Stone pile vs Control)",
      RQ == "RQ1"  & comparison == "reference"                    ~ "Baseline check (Ref 1 vs Ref 2)",
  ...
  dplyr::arrange(
    factor(group, levels = c(
      "Baseline check (Stone pile vs Control)",
      "Baseline check (Ref 1 vs Ref 2)",
...
ggsave(file.path(out_dir, "fig-forest-plot.png"), forest_plot, width = 4, height = 14, dpi = 300)
...
m_bayes <- brm(
  `Weighted_CPUE_Kōura` ~ site * monitoring_int + (1 | pair_id/site_id),
  data   = rq2b_data,
  family = hurdle_gamma(),
  chains = 4,
  iter   = 2000,
...
hypothesis(m_bayes, "siteStone_pile:monitoring_int > 0")
```

```{r}
library(unmarked)

# Step 1: detection per site × round × net type
koura_det <- Fish_data |>
  dplyr::mutate(
    monitoring_int = as.integer(stringr::str_extract(monitoring_id, "(?<=_)\\d+"))
  ) |>
  dplyr::group_by(site_id, monitoring_int, net_type) |>
  dplyr::summarise(
    detected = as.integer(any(is_koura, na.rm = TRUE)),
    .groups = "drop"
  )

# Step 2: wide format — one column per net type per round
det_wide <- koura_det |>
  tidyr::pivot_wider(
    names_from  = net_type,
...
# Step 3: join site covariates
site_covs <- Combined_data |>
  dplyr::distinct(site_id, site, pair_id) |>
  dplyr::mutate(site_id = as.character(site_id),
                site     = factor(site, levels = c("Control", "Stone_pile", "Reference")))

occ_data <- det_wide |>
  dplyr::mutate(site_id = as.character(site_id)) |>
  dplyr::left_join(site_covs, by = "site_id") |>
  dplyr::arrange(site_id, monitoring_int)

# Step 4: build unmarked frame
# detection matrix: Box and Fyke as two occasions
y_mat <- occ_data |>
  dplyr::select(Box, Fyke) |>
  as.matrix()

site_df <- occ_data |>
  dplyr::select(site, pair_id, monitoring_int) |>
...
# occupancy ~ site type + monitoring round
# detection ~ net type (estimated from Box vs Fyke difference)
m_occ <- occu(~ 1 ~ site + monitoring_int, data = umf)
summary(m_occ)

# predicted occupancy by site type
predict(m_occ, type = "state",
        newdata = data.frame(
          site           = factor(c("Control","Stone_pile","Reference"),
                                  levels = c("Control","Stone_pile","Reference")),
          monitoring_int = factor("1")))

```

# Multi-scale hierarchical framework
```{r}
#| label: Multi-scale-hierarchical-framework

library(vegan)

# Two-scale variance partition (Scale 1: location, Scale 2: habitat)
vp_data <- site_summary |>
  dplyr::mutate(pair_id = as.factor(pair_id)) |>
  dplyr::filter(!is.na(substrate_after))

nrow(vp_data)

# X1 — lake/location scale: pair identity as spatial block
X1 <- model.matrix(~ pair_id - 1, data = vp_data)

# X2 — site/habitat scale: substrate index only
X2 <- vp_data |>
  dplyr::select(substrate_after) |>
  dplyr::mutate(across(everything(), scale)) |>
  as.data.frame()

Y <- log1p(vp_data$mean_cpue)

# Variance partition
vp <- vegan::varpart(Y, X1, X2)
vp
plot(vp, digits = 2, bg = c("#E8A598", "#A8C5A0"))

# Partial RDA significance tests
# [a] pure location: X1 | X2
rda_a <- vegan::rda(Y ~ pair_id + Condition(substrate_after), data = vp_data)
anova(rda_a, permutations = 9999)

# [b] pure habitat: X2 | X1
rda_b <- vegan::rda(Y ~ substrate_after + Condition(pair_id), data = vp_data)
anova(rda_b, permutations = 9999)

```

## Dynamic model — Scale 3 temporal/biotic predictors
```{r}
#| label: dynamic-model

# ── Data preparation ───────────────────────────────────────────────────────
# Size-weighted catfish predation pressure (Barnes & Hicks 2003):
# large >250mm = 64% kōura in gut, small <150mm = 15%, medium = 35%
catfish_pressure <- Fish_data |>
  dplyr::filter(species == "Catfish", !is.na(length_mm)) |>
  dplyr::mutate(
    site_id          = as.factor(site_id),
    predation_weight = dplyr::case_when(
      length_mm > 250 ~ 0.64,
      length_mm < 150 ~ 0.15,
      TRUE            ~ 0.35)) |>
  dplyr::group_by(site_id, monitoring_id) |>
  dplyr::summarise(
    weighted_catfish_cpue = sum(predation_weight) / dplyr::n(),
    .groups = "drop")

dynamic_data <- all_data |>
  dplyr::filter(monitoring_int >= 1, !is.na(`Weighted_CPUE_Kōura`)) |>
  dplyr::left_join(catfish_pressure, by = c("site_id", "monitoring_id")) |>
  dplyr::mutate(
    site_id        = factor(site_id),
    catfish_cpue_c = scale(`Weighted_CPUE_Catfish`)[,1],
    temp_c         = scale(Temperature)[,1])

# ── Global model: only time-varying predictors as fixed effects ────────────
# substrate, wood, site type absorbed by (1|site_id) random intercept
m_dynamic <- glmmTMB::glmmTMB(
  `Weighted_CPUE_Kōura` ~ Season + temp_c + catfish_cpue_c + (1 | site_id),
  data      = dynamic_data,
  REML      = FALSE,
...
# ── Model selection ────────────────────────────────────────────────────────
dredge_dynamic <- MuMIn::dredge(m_dynamic)
dredge_dynamic

# Model-averaged coefficients (models within 4 AICc of top)
avg_dynamic <- MuMIn::model.avg(dredge_dynamic, subset = delta < 4)
summary(avg_dynamic)

# ── Site-level random effects (BLUPs) ─────────────────────────────────────
# Residual between-site variation after accounting for temporal predictors
site_ranef <- glmmTMB::ranef(m_dynamic)$cond$site_id |>
  tibble::rownames_to_column("site_id") |>
  dplyr::rename(site_effect = `(Intercept)`)

site_ranef |> dplyr::arrange(dplyr::desc(site_effect))

# ── Catfish × habitable depth interaction ──────────────────────────────────
# Tests whether catfish predation pressure depends on habitat depth
# (requires habitable_depth_m — check if column exists in all_data)
m_interaction <- glmmTMB::glmmTMB(
  `Weighted_CPUE_Kōura` ~ scale(`Weighted_CPUE_Catfish`) * scale(habitable_depth_m) +
    Season + (1 | site_id),
  data   = dynamic_data |> dplyr::filter(!is.na(habitable_depth_m)),
  REML   = FALSE,
  family = glmmTMB::tweedie())
