...
#| echo: false
#| eval: !expr knitr::is_html_output()
#| fig-cap: "Effect sizes (β ± 95% CI) for the key term of interest across all models, for kōura (left) and catfish (right). Filled points indicate significant effects (*p* < 0.05); open points are non-significant. Hover over points for exact estimates, confidence intervals, and p-values. Each panel has its own x-axis scale."
ch5_dir <- if (dir.exists("05_stone_piles_rotoiti")) "05_stone_piles_rotoiti" else "."
.rds <- file.path(ch5_dir, "data/derived/widget_forest_plot.rds")
if (file.exists(.rds)) readRDS(.rds) else htmltools::tags$p(htmltools::tags$em("Interactive plot not yet built — render analysis.qmd first."), style = "color:#888;padding:1em")
```
:::
