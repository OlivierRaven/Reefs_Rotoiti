...
PLOT <- Monitoring_CPUE_data %>%
  ggplot(aes(as.factor(Monitoring),CPUE_Kōura, fill = Site)) +
...
# Morihana
p1_morihana <- Monitoring_CPUE_data %>%
  filter(Site != "Reference") %>%
  mutate(Monitoring_label = factor(Monitoring_label,levels = c("0 Pre/Summer", "1 Autumn", "2 Winter", "3 Spring"))) %>%
  ggplot(aes(Monitoring_label,`Total_Individuals_Morihana`, col = Site)) +
  geom_point(size = 2) +
  geom_smooth(se = FALSE, aes(group = Site)) +
  facet_wrap( ~ ID, ncol=2) +
  scale_colour_manual(values = c("Reef" = "red", "Control" = "orange")) +
  labs(y = "Total Morihana", x = "Monitoring") +
  theme_bw()

p2_morihana <- Monitoring_CPUE_data %>%
  filter(Site == "Reference") %>%
  mutate(Monitoring_label = factor(Monitoring_label,levels = c("0 Pre/Summer", "1 Autumn", "2 Winter", "3 Spring"))) %>%
  ggplot(aes(Monitoring_label,`Total_Individuals_Morihana`, col = Site)) +
  geom_point(size = 2) +
...
combined_morihana <- p1_morihana + p2_morihana + plot_layout(guides = "collect") & theme(legend.position = "bottom")
combined_morihana
