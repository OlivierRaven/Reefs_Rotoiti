...
run_contrasts <- function(fit, model_data, model_label, include_season = TRUE) {
  ...
  season_shapes  <- c(Summer = 16, Autumn = 17, Winter = 15, Spring = 18)
  ...
  shape_scale <- if (include_season)
    ...
    scale_shape_manual(values = c("After treatment" = 16), guide = "none")
  ...
  make_forest <- function(data) {
    ...
    ggplot(data, aes(x = mean, y = contrast,
                     ...
                     shape  = if (include_season) season else NULL)) +
      ...
      colour_scale + shape_scale + x_scale + shared_theme +
